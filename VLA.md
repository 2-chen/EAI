...
* 一次性预测多个动作，提升推理速度，实现实时控制
* FiLM：在视觉编码器的每一层中，利用仿射变换根据语言嵌入来调制视觉特征图，强迫模型关注与指令相关的视觉区域，解决模型容易忽略语言指令的问题
  * 比如指令中提到红色，就会放大视觉特征图中负责红色通道的数值
* 双向注意力掩码用于并行解码：比如一次性输出未来25步的动作，第1步动作和第25步动作是同时在计算的，能互相看见并交流，此时轨迹才是一条协调的整体
