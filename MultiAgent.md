...
* 代理式AI适配策略的统一分类框架：四大适配范式
  * A1：基于工具执行信号的agent适配，agent生成调用代码，工具执行后将结果作为奖励信号更新agent参数，看过程，即工具的结果不是整体的结果
  * A2：基于agent输出信号的agent适配，agent使用工具产生最终答案作为奖励信号更新agent参数，看结果
  * T1：agent无关的工具适配，使用预训练好的工具（检索器、计算器、API），通用型
