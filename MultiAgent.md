...
* 基于谱聚类的关键关系提取：识别和提炼出机器人之间、机器人与物体之间最重要、最持久的交互关系
  * 谱聚类：将所有数据点看作图中的节点，试图找到一种切割图的方式，使得被切开的各个子图内部连接紧密，而子图间连接稀疏，交互频率影响每条边的权重
...
*想法*：对于一个原始数据，应该分别学习其内部不同特征，而不是全部混在一起学习，容易互相干扰
