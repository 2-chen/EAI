...
* 在推理阶段动态生成智能体角色并实时调整沟通网络（拓扑结构），且不需要对底层大模型进行微调
* 引入Architect Agent接收新查询时根据任务需求生成候选角色，基于向量嵌入的相似度进行过滤
* 任务内演化：在执行过程中，如果收集到报错日志或发现某个角色的输出毫无用处，重写该角色的提示词，或者动态切断/重连节点之间的通信边，让拓扑结构适应当前困难
* 任务间演化：一个任务成功完成后，MetaGen 会计算一个包含成功率和 Token 消耗的综合Reward，系统利用这个奖励来更新角色和连接的选择权重，将那些在任务中表现出色且新颖的角色“固化”保存到一个缓存库中，供未来的相似任务直接复用，从而实现跨任务的经验积累
* 免训练是一大优势
* 问题：
  * 依赖底层模型能力：极度依赖Architect Agent凭空生成高质量角色和重写提示词的能力
  * 推理延迟增加：任务内演化会增加系统响应的时间延迟
  * 拓扑结构的局限：强制生成有向无环图DAG排除了具有积极作用的多轮循环辩论或持续反思闭环
  * 反馈信号的适用范围：依赖于明确的执行反馈来触发演化，对于没有明确客观评价标准的开放式生成任务难以提取高质量的反馈信号来指导拓扑更新
...
* 代理式AI适配策略的统一分类框架：四大适配范式
  * A1：基于工具执行信号的agent适配，agent生成调用代码，工具执行后将结果作为奖励信号更新agent参数，看过程，即工具的结果不是整体的结果，反馈信号具有高度确定性
  * A2：基于agent输出信号的agent适配，agent使用工具产生最终答案作为奖励信号更新agent参数，看结果，适用于开放性、主观性较强的任务，缺乏明确可执行验证的场景
  * T1：agent无关的工具适配，使用预训练好的工具（检索器、计算器、API），通用型，agent学习调度和组合
  * T2：agent监督的工具适配，利用agent的反馈来训练工具，定制型
...
* 协同适配：不在固定其中一方，而是agent的决策和工具的内部机制同步调整，彼此构成动态的学习环境，但存在责任归因问题
